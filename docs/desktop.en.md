# NovaPaw Desktop Application Guide
...
1. **Download the installer**
   Download `NovaPaw-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\NovaPaw`
...
#### **NovaPaw Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "NovaPaw Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **NovaPaw Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "NovaPaw Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "NovaPaw Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "NovaPaw Desktop" → Uninstall
...
1. **Download the archive**
   Download `NovaPaw-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `NovaPaw.app`

3. **Move to Applications folder (Optional)**
   Drag `NovaPaw.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

NovaPaw is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `NovaPaw.app`
...
2. Scroll down to find a message like:
   _"'NovaPaw' was blocked from use because it cannot verify the developer"_
...
xattr -cr /Applications/NovaPaw.app
```
...
- Double-click `NovaPaw.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.novapaw/desktop.log`
...
cd /Applications  # or wherever your NovaPaw.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/NovaPaw.app/Contents/Resources/env"
...
1. Check the `~/.novapaw/desktop.log` file for errors
...
**Q: How to uninstall?**
A: Drag `NovaPaw.app` to the Trash, then delete the `~/.novapaw` configuration folder
