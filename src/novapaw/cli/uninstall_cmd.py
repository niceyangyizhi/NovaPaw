...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove NovaPaw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# NovaPaw" comment line and the export PATH line
    cleaned = re.sub(
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove NovaPaw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL NovaPaw data in {wd}")
    else:
        click.echo(
            "This will remove the NovaPaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("NovaPaw uninstalled. Please restart your terminal.")
