...
from ..utils.logging import setup_logger, add_novapaw_file_handler
...
async def lifespan(
    ...
    add_novapaw_file_handler(WORKING_DIR / "novapaw.log")
    await runner.start()
...
def _resolve_console_static_dir() -> str:
    ...
    # because the console will be output to novapaw's
    # `src/novapaw/console/` directory directly by vite.
    cwd = Path(os.getcwd())
...
def read_root():
    ...
    return {
        "message": (
            "NovaPaw Web Console is not available. "
            "If you installed NovaPaw from source code, please run "
            "`npm ci && npm run build` in NovaPaw's `console/` "
            "directory, and restart NovaPaw to enable the web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current NovaPaw version."""
