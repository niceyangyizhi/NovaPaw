...
class NovaPawAgent(ToolGuardMixin, ReActAgent):
    ...
    Python's MRO: NovaPawAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_novapaw_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_novapaw_rebuild_info", rebuild_info)
                return rebuilt_client
