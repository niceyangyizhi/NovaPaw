...
$Archive = Join-Path $Dist "novapaw-env.zip"
$Unpacked = Join-Path $Dist "win-unpacked"
$NsiPath = Join-Path $PackDir "novapaw_desktop.nsi"
...
$VersionFile = Join-Path $RepoRoot "src\novapaw\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "novapaw-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "novapaw-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "NovaPaw Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var NOVAPAW_LOG_LEVEL or default to "info"
if not defined NOVAPAW_LOG_LEVEL set "NOVAPAW_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\novapaw_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.novapaw\config.json" (
  "%~dp0python.exe" -u -m novapaw init --defaults --accept-security
)
"%~dp0python.exe" -u -m novapaw desktop --log-level %NOVAPAW_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "NovaPaw Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with NOVAPAW_LOG_LEVEL)
if not defined NOVAPAW_LOG_LEVEL set "NOVAPAW_LOG_LEVEL=debug"
...
echo ====================================
echo NovaPaw Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo Log Level: %NOVAPAW_LOG_LEVEL%
...
echo [Launch] Starting NovaPaw Desktop with log-level=%NOVAPAW_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m novapaw desktop --log-level %NOVAPAW_LOG_LEVEL%
echo.
echo [Exit] NovaPaw Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "NovaPaw Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\NovaPaw Desktop.bat"
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('novapaw'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] NOVAPAW_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "NovaPaw-Setup-$Version.exe"
